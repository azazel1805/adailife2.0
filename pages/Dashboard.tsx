...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate, allTabs, adaiMenuStructure = defaultAdaiMenuStructure }) => { // Use default value here
  ...
  const [favoriteTabs, setFavoriteTabs] = useLocalStorage<Tab[]>(`favorite-tabs-${user?.uid || 'guest'}`, []); // Use user.uid for unique key
  ...
  const WelcomeHeader = () => (
    <div className="flex justify-between items-center">
        <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">Welcome!</p>
        </div>
    </div>
  );
