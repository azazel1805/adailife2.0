    ...
    const renderQuiz = () => (
        <div className="space-y-6">
            {questions.map((q, index) => {
                ...
                return (
                <div key={index}>
                    <p className="font-semibold text-slate-900 dark:text-slate-200 mb-4"><span className="text-brand-primary">{index + 1}.</span> {q.question}</p>
            ...
            {sources.map((source, index) => (
                <li key={index} className="flex items-start">
                    ...
                    <a 
                        ...
                        className="text-slate-500 dark:text-slate-400 hover:text-brand-primary hover:underline break-all"
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-lg">
                <h2 className="text-2xl font-bold mb-2 text-slate-900 dark:text-slate-200">Etkileşimli Gündem</h2>
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-md font-semibold transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${
                                topic === category && !(isLoading || isLoadingQuestions)
                                    ...
                                    : 'bg-gray-100 hover:bg-gray-200 text-slate-500 dark:text-slate-400'
                            }`}
                        >
                            {category}
                        </button>
            ...
            {paragraph && (
                <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-lg space-y-4">
                    ...
                    <p className="leading-relaxed text-justify text-slate-900 dark:text-slate-200">{paragraph}</p>
                </div>
            )}
                        ...
                        <nav className="-mb-px flex space-x-4" aria-label="Tabs">
                             <button
                                ...
                                activeView === 'quiz'
                                    ...
                                    : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-slate-200 hover:border-gray-300'
